 ...
 * @date 2020/9/10 9:09
 */
public class Handle00262_00057400 implements CtrlHandleInter {
    ...
    public ModelMap action(AppApiCommonForm form1, ModelMap modelMap) throws CenterRuntimeException, Exception {
        ...
        log.debug("00057400请求00262参数："+ CommonUtil.getStringParams(form));
        ...
        if(Constants.method_BSP.equals(send)){
            ...
            String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REQ_00262_128932.txt", map, req);
            log.debug("前置YFMAP发往中心报文："+xml);

            String ip = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_IP"+form.getCenterId()).trim();
            String port = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_PORT"+form.getCenterId()).trim();
            String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "128932");
            log.debug("前置YFMAP接收中心报文——合同变更申请前查询申请："+rexml);
            HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REP_00262_128932.txt", rexml, req);
