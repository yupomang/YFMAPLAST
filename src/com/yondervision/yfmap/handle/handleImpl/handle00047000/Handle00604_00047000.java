...
import com.yondervision.yfmap.result.hulunbuir.HulunbuirAppApi00604Result;
...
public class Handle00604_00047000 implements CtrlHandleInter {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap) throws CenterRuntimeException,Exception{
		/* 模拟返回开始  */	
		log.info(Constants.LOG_HEAD+"appApi00604 start.");
		AppApi00601Form form = (AppApi00601Form)form1;
		List<HulunbuirAppApi00604Result> list = new ArrayList<HulunbuirAppApi00604Result>();
		...
		String fname = formatter2.format(date)+"_"+form.getSurplusAccount()+"_"+"api00605DKHTH.txt";
		AES aes = new AES();
		form.setIdcardNumber(aes.decrypt(form.getIdcardNumber()));
		HulunbuirAppApi00604Result app006Result = new HulunbuirAppApi00604Result();
		if(Constants.method_BSP.equals(send)){////xml通信处理
			...
			String reqxml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "128806");
...
//			String reqxml = "<AuthCode1>IVR0</><AuthCode2>IVR0</><AuthCode3>IVR0</><AuthFlag>0</><BrcCode>00000000</><BusiSeq>90</><ChannelSeq>398243</><ChkCode>cweb</><MTimeStamp>2013-07-24 09:55:28:967000</><NoteMsg></><RspCode>000000</><RspMsg>成功</><STimeStamp>2015-07-24 09:55:28:966000</><TellCode>cweb</><TranChannel>00</><TranCode>116005</><TranDate>2016-04-06</><TranIP>10.22.21.26</><TranSeq>90</><fileName>YEMXCX.txt</><accnum></><accname></><sumcounts></><counts></><pageflag>1</>";
			...
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REP_DKHTH.txt", reqxml, req);
			log.debug("解析报文MAP："+resultMap);
			BeanUtil.transMap2Bean(resultMap, app006Result);
			log.debug("MAP封装成BEAN："+app006Result);
			if(!Constants.sucess_ts.equals(app006Result.getRecode())){
			...
			if(!app006Result.getFileName().equals("")){
				String filename = app006Result.getFileName().split("/")[app006Result.getFileName().split("/").length-1];
				app006Result.setFileName(filename);
...
//				}
				while (line != null) {
					HulunbuirAppApi00604Result aa00604Result = new HulunbuirAppApi00604Result();
					try {
//						if(pnum<Integer.valueOf(form.getPagerows())){
//							if(sum>num){
								String[] param = line.split("~");									
								log.debug("读取文件  ："+line);
								aa00604Result.setLoancontrnum(param[0].trim());						
								aa00604Result.setAccname(param[1].trim());
								aa00604Result.setCertinum(param[2].trim());						
								aa00604Result.setPubaccnum(param[3].trim());
								if(param.length>4){
								aa00604Result.setBegdate(param[4].trim());
								}
								list.add(aa00604Result);
//								pnum++;
//							}
//						}
//						sum+=1;
