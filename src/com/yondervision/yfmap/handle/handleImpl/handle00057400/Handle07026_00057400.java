...
public class Handle07026_00057400 implements CtrlHandleInter {
...
//				form.setBrcCode(PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "appapi001BrcCode"+form.getCenterId()).trim());
		    ...
		    String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REQ_07026_128118.txt", map, req);
			log.debug("前置YFMAP发往中心报文："+xml);
			String ip = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_IP"+form.getCenterId()).trim();
			String port = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_PORT"+form.getCenterId()).trim();
			String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "128118");
			log.debug("前置YFMAP接收中心报文——贷款还款记录查询："+rexml);
		    HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REP_07026_128118.txt", rexml, req);
		...
		modelMap.put("msg", "成功");
		modelMap.put("rsufilename", app07010ZHResult.getRsufilename());
		modelMap.put("rsufilepath", app07010ZHResult.getRsufilepath());
		modelMap.put("consum", app07010ZHResult.getConsum());
		return modelMap;
	}
}
