...
public class Handle011_00043200 implements CtrlHandleInter {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap)
		...
		AES aes= new AES();
		try{
			form.setFreeuse1(aes.decrypt(form.getFreeuse1()).trim());
		...
		if(Constants.method_XML.equals(send)){////xml通信处理
			...
			String reqxml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "B00003");
...
//			String reqxml = "<TranCode>600027</><TranDate>2014-04-21</><STimeStamp>2014-05-13 09:33:41</><MTimeStamp>2014-05-13 09:33:41</><BrcCode>00063000</><TellCode>6000</><ChkCode>1231</><AuthCode1>1231</><AuthCode2>1231</><AuthCode3>1231</><TranChannel>11</><TranIP>192.168.3.221</><ChannelSeq>-4926</><TranSeq>0</><BusiSeq>0</><RspCode>000000</><RspMsg>交易处理成功...</><NoteMsg></><AuthFlag>123112310</><FinancialDate>2014-01-05</><HostBank>aaa</><SubBank>aaa</><InstCode>00063100</><AccName1>迟  健</><ApprLoanSum>140000.00</><ApprLoanTerm>15</><TransDate>2002-09-26</><ApprNum>00200200201</><ApprFlagC>15</><ApprFlagN>15</><HouseAddr>庙讲路61号304</>";
			...
			if(!"00".equals(app011Result.getRecode())){
				modelMap.clear();
		...
		for(int i=1;i<14;i++){
			...
			if(PropertiesReader.getProperty("yingshe.properties", form.getCenterId()+"_apprflagstat_"+num)==null)continue;
			val = PropertiesReader.getProperty("yingshe.properties", form.getCenterId()+"_apprflagstat_"+num).trim();
			state = PropertiesReader.getProperty("yingshe.properties", form.getCenterId()+"_apprflag_"+val).trim();
			if(list.size()==0){				
				AppApi011Result app011 = new AppApi011Result();
				app011.setApprflagID(val);
				app011.setApprflagMSG(state);
			...
			if(!val.equals("")){
				for(int j=0;j<list.size();j++){
					...
					if(list.get(j).getApprflagMSG().equals(PropertiesReader.getProperty("yingshe.properties", form.getCenterId()+"_apprflag_"+val).trim())){						
						isflag = true;
						break;
					}
				}
		...
		String apprflagMSG = PropertiesReader.getProperty("yingshe.properties", form.getCenterId()+"_apprflag_"+PropertiesReader.getProperty("yingshe.properties", form.getCenterId()+"_apprflagstat_"+app011Result.getApprflag()).trim()).trim();
		modelMap.clear();
		if("10".equals(app011Result.getApprflag())){
			modelMap.put("appflagStat", "1");
		}else{
			modelMap.put("appflagStat", "0");
		}
		modelMap.put("recode", "000000");
		modelMap.put("msg", "成功");
		modelMap.put("apprflagID", PropertiesReader.getProperty("yingshe.properties", form.getCenterId()+"_apprflagstat_"+app011Result.getApprflag()).trim());
