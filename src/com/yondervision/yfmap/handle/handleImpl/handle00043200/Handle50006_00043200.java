...
public class Handle50006_00043200 implements CtrlHandleInter {
...
//		form.setPassword(new String(c));
		...
		if(Constants.method_XML.equals(send)){////xml通信处理
			...
			if(form.getFreeuse1()!=null&&form.getFreeuse1().equals("sendsms")){
				String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/REQ_DXYZ.xml", map, req);
				log.debug("前置YFMAP发往中心报文："+xml);			
				String ip = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_IP"+form.getCenterId()).trim();
				String port = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_PORT"+form.getCenterId()).trim();
				String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "C040009</tr_code");
				log.debug("前置YFMAP接收中心报文："+rexml);
				HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/REP_DXYZ.xml", rexml, req);
