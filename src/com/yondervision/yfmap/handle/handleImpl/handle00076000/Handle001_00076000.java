...
public class Handle001_00076000 implements CtrlHandleInter {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap) throws CenterRuntimeException,Exception{
		...
		if(Constants.method_XML.equals(send)){////xml通信处理
			...
			String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "119012");
...
//			String rexml = "<?xml version=\"1.0\" encoding=\"GBK\"?><root><head><transCode>appapi001</transCode><recvDate>2015-07-30</recvDate><recvTime>14:30:30</recvTime><sendSeqno>12345678</sendSeqno><key></key><centerSeqno>123456</centerSeqno><recode>000000</recode><msg>success</msg></head><body><accnum>801078393844</accnum><accname>陈倩敏</accname><statename>正常</statename><certinum>442000199107280546</certinum><bal>0.00</bal><lpaym></lpaym><lasttransdate>1899-12-31</lasttransdate><increbal>10000.00</increbal><keepbal>2345.00</keepbal><basenum>3000</basenum><unitprop>0.120</unitprop><indiprop>0.12</indiprop><unitaccnum>12345678</unitaccnum><unitaccname>as东方时代</unitaccname><accinstcode>123456</accinstcode><opnaccdate>1899-12-31</opnaccdate><freeuse1></freeuse1><freeuse2></freeuse2><freeuse3></freeuse3></body></root>";
		...
		if(Double.parseDouble(app40101Result.getCompanyPaymentRate().replaceAll("%", ""))<1){
			app40101Result.setCompanyPaymentRate(Double.parseDouble(app40101Result.getCompanyPaymentRate().replaceAll("%", ""))*100+"%");
		}
		if(Double.parseDouble(app40101Result.getPersonalPaymentRate().replaceAll("%", ""))<1){
			app40101Result.setPersonalPaymentRate(Double.parseDouble(app40101Result.getPersonalPaymentRate().replaceAll("%", ""))*100+"%");
		}
