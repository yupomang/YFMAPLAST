...
import com.yondervision.yfmap.util.DESForJava_CD;
import com.yondervision.yfmap.util.JavaBeanUtil;
import com.yondervision.yfmap.util.PropertiesReader;

public class Handle500_00083800 implements CtrlHandleInter {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap)
		...
		byte[] b = DESForJava_CD.encryption(form.getNewpassword(), "12345678");
		byte[] c = DESForJava_CD.bcd_to_asc(b);
		...
		if(Constants.method_BSP.equals(send)){////xml通信处理
			...
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/REQ_DL.txt", map, req);
			log.debug("前置YFMAP发往中心报文："+xml);			
			String ip = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_IP"+form.getCenterId()).trim();
			String port = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_PORT"+form.getCenterId()).trim();
			String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "430002");
//			String rexml ="<?xml version=\"1.0\" encoding=\"GBK\"?><root><head><transCode></transCode><recvDate></recvDate><recvTime></recvTime><sendSeqno>zsgjiapp20141021172832745</sendSeqno><key>1</key><centerSeqno></centerSeqno><recode>000000</recode><msg>error</msg></head><body><accnum>0010001001</accnum><accname>kmcs</accname><certinum>220180980909120</certinum><freeuse1></freeuse1><freeuse2></freeuse2><freeuse3></freeuse3></body></root>";
			log.debug("前置YFMAP接收中心报文："+rexml);
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/REP_DL.txt", rexml, req);
