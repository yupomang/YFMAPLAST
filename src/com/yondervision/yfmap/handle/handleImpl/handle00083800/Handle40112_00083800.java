 ...
 * 请求验证码
 */
public class Handle40112_00083800 implements CtrlHandleInter {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap)
		...
		if(Constants.method_BSP.equals(send)){
			...
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"REQ_SQYZM.txt", map, req);
			...
			String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "110091");

			log.debug("中心下传报文："+rexml);
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"REP_SQYZM.txt", rexml, req);
//			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, "D:/server/apache-tomcat-6.0.44/webapps/ydmi_csp/WEB-INF/classes/com/yondervision/yfmap/config/messagetemp/00053100/REP_SQYZM.xml", rexml, req);
			...
			if(!"0".equals(app01003Result.getRecode())){
	...
	public static void main(String[] args){
		...
		Handle40112_00083800 hand = new Handle40112_00083800();
		try {
