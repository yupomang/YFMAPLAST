...
public class Handle002_00057400 implements CtrlHandleInter {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap)
		...
		log.debug("00057400请求00201参数："+CommonUtil.getStringParams(form));
		...
		if(Constants.method_BSP.equals(send)){
			...
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REQ_GRZHMXCX.txt", map, req);
			
			log.debug("前置YFMAP发往中心报文："+xml);			
			String ip = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_IP"+form.getCenterId()).trim();
			String port = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_PORT"+form.getCenterId()).trim();
			String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "142504");
						
			log.debug("前置YFMAP接收中心报文："+rexml);
			resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"BSP_REP_GRZHMXCX.txt", rexml, req);			
			...
			if(!CommonUtil.isEmpty(filename)){				
				...
				while (line1 != null) {// 判断读取到的字符串是否不为空
					try {							
						...
						app002_01ZHResult.setDwmc(trs[0].trim());
						app002_01ZHResult.setJzrq(trs[1].trim());
						app002_01ZHResult.setJymc(trs[2].trim());
						app002_01ZHResult.setFse(trs[3].trim());
						app002_01ZHResult.setBal(trs[4].trim());
