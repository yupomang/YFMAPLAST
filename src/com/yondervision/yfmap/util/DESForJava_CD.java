...
public class DESForJava_CD {
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		...
		// 密钥的加密解密
		byte[] encKey = DESForJava_CD.keydes("12345678".getBytes(), "12345678".getBytes(), 8, 0);
		String encKeyS = new String(encKey);
		System.out.println(">>>"+encKeyS);
		byte[] decKey = DESForJava_CD.keydes(encKeyS.getBytes(), "12345678".getBytes(), 8, 1);
		System.out.println(new String(decKey));
		
		String key = "12345678";
		
		byte[] b = DESForJava_CD.encryption("000000  ", key);
		byte[] c=bcd_to_asc(b);
		System.out.println(">>>>>ssss"+new String(c));
		int i = c.length;
		byte[] d=asc_to_bcd(c,i) ;
		String kk = DESForJava_CD.decryption(d, key);
		...
		b = DESForJava_CD.encryption(kkk, "1234567812345678");
		kk = DESForJava_CD.decryption(b, "1234567812345678");
