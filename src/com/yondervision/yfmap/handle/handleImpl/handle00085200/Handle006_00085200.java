...
/** 
* @ClassName: Handle006_00085200 
* @Description: 128029-个人贷款信息查询
* @author Caozhongyan
* @date Apr 8, 2016 11:23:09 AM   
* 
*/ 
public class Handle006_00085200 implements CtrlHandleInter {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap)
		...
		log.debug("form:"+form); 
		ZunYiAppApi00601Result app00601Result= new ZunYiAppApi00601Result();
		if(Constants.method_BSP.equals(send)){
			...
			String reqxml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "128029"); 
...
//			String reqxml = "<AuthCode1>IVR0</><AuthCode2>IVR0</><AuthCode3>IVR0</><AuthFlag>0</><BrcCode>00000000</><BusiSeq>90</><ChannelSeq>398243</><ChkCode>cweb</><MTimeStamp>2013-07-24 09:55:28:967000</><NoteMsg></><RspCode>000000</><RspMsg>OK</><STimeStamp>2013-07-24 09:55:28:966000</><TellCode>cweb</><TranChannel>00</><TranCode>128029</><TranDate>2014-12-11</><TranIP>10.22.21.26</><TranSeq>90</><fileName></><certinum>1</><accname>王小二1</><loancontrstate>1</><loanamt>250000.00</><loanterm>120</><repaymode>1</><agentbankcode>1</><loandate>2015-03-21</><repayday>10</><enddate>2025-03-21</><repayprinsum>1000.00</><repayintsum>1</><loanbal>240000.00</><loancontrnum>29182098129121212</><apprloanamt>1</><loaneeaccname>1</><loanrate>3.5</><handset>1</><repayplansum>1</><repayplanprin>1</><repayplanint>1</><oweprin>0.00</><oweint>0.00</><owepun>1</><repaypirn>1</><repayint>1</><repaysum>1</><owesum>1</><begdate>1</><apprenddate>1</><monthrepayamt>1</><loanhousenum>1</><loancontrcode>1</><remainterm>1</><owecnt>0</><termnum>10</><cleardate>1</><loanrelation>1</><seqnum>10</><famt0>2000.00</><famt1>1590.00</><famt2>1890.00</><famt3>2152.00</><famt4>2519.00</><famt5>2988.00</><famt6>1198.00</><famt7>9882.00</><famt8>2812.00</><summary>公积金等额本金</><summary1>建设银行</>";
		...
		app00601Result.setRepayprinsum(String.format("%,.2f",Double.valueOf(app00601Result.getRepayprinsum())));
		app00601Result.setFamt8(String.format("%,.2f",Double.valueOf(app00601Result.getFamt8())));
		app00601Result.setLoanbal(String.format("%,.2f",Double.valueOf(app00601Result.getLoanbal())));
		app00601Result.setFamt7(String.format("%,.2f",Double.valueOf(app00601Result.getFamt7())));
		app00601Result.setFamt1(String.format("%,.2f",Double.valueOf(app00601Result.getFamt1())));
		app00601Result.setFamt2(String.format("%,.2f",Double.valueOf(app00601Result.getFamt2())));
		app00601Result.setFamt3(String.format("%,.2f",Double.valueOf(app00601Result.getFamt3())));
		app00601Result.setFamt4(String.format("%,.2f",Double.valueOf(app00601Result.getFamt4())));
		app00601Result.setFamt5(String.format("%,.2f",Double.valueOf(app00601Result.getFamt5())));
		app00601Result.setFamt6(String.format("%,.2f",Double.valueOf(app00601Result.getFamt6())));
		...
		app00601Result.setOwecnt(String.format("%,.2f",Double.valueOf(app00601Result.getOwecnt())));
		
		if("0".equals(app00601Result.getLoanrelation())){
			app00601Result.setLoaneeaccname(app00601Result.getLoaneeaccname()+"-本人");
		}else if("1".equals(app00601Result.getLoanrelation())){
			app00601Result.setLoaneeaccname(app00601Result.getLoaneeaccname()+"-夫妻");
		}else if("2".equals(app00601Result.getLoanrelation())){
			app00601Result.setLoaneeaccname(app00601Result.getLoaneeaccname()+"-父子");
		}else if("3".equals(app00601Result.getLoanrelation())){
			app00601Result.setLoaneeaccname(app00601Result.getLoaneeaccname()+"-母子");
		}else if("9".equals(app00601Result.getLoanrelation())){
			app00601Result.setLoaneeaccname(app00601Result.getLoaneeaccname()+"-其他");
		}
