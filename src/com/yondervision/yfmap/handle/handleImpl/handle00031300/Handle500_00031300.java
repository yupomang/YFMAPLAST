...
import com.yondervision.yfmap.result.zhangjiakou.ZhangJiaKouAppApi40101Result;
 ...
 *
 */
public class Handle500_00031300 implements CtrlHandleInter {
...
//		form.setNewpassword(aes.decrypt(form.getNewpassword()));
		...
		ZhangJiaKouAppApi40101Result app50001Result = new ZhangJiaKouAppApi40101Result();//
		if(Constants.method_XML.equals(send)){////xml通信处理
			...
			form.setFullName(aes.decrypt(form.getFullName()));         //名字使用aes加密
//			form.setFullName(form.getFullName());
			form.setIdcardNumber(aes.decrypt(form.getIdcardNumber())); //身份证使用aes加密
//			form.setIdcardNumber(form.getIdcardNumber());
			...
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/REQ_BD.xml", map, req);
//	    	String xml = MessageCtrMain.encapsulatedPackets(msgType, "D:/tools/tomcat6.0.14/tomcat6.0.14/webapps/ydmi_csp/WEB-INF/classes/com/yondervision/yfmap/config/messagetemp/00031300//REQ_BD.XML", map, req);
...
//				"</body></root>";
			...
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/REP_BD.xml", rexml, req);
//			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, "C:/Program Files/Tomcat 6.0/webapps/YFMAP/WEB-INF/classes/com/yondervision/yfmap/config/messagetemp/00053100/REP_YZ.xml", rexml, req);
			...
			BeanUtil.transMap2Bean(resultMap, app50001Result);
			log.debug("MAP封装成BEAN："+app50001Result);
			if(!Constants.sucess_ts.equals(app50001Result.getRecode())){
				modelMap.put("recode", app50001Result.getRecode());
				modelMap.put("msg", app50001Result.getMsg());
				log.error("中心返回报文 状态recode :"+app50001Result.getRecode()+"  ,  描述msg : "+app50001Result.getMsg());
				return modelMap;
			}
		}
