...
import com.yondervision.yfmap.result.hulunbuir.HulunbuirAppApi00701Result;
...
public class Handle007_00047000 implements CtrlHandleInter {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap) throws CenterRuntimeException,Exception{
		...
		List<HulunbuirAppApi00701Result> list = new ArrayList<HulunbuirAppApi00701Result>();
		...
		String fname = formatter2.format(date)+"_"+form.getSurplusAccount()+"_"+"api00701GJJMX.txt";
		AES aes = new AES();
		form.setPubaccnum(aes.decrypt(form.getPubaccnum()));
		HulunbuirAppApi00701Result app002Result = new HulunbuirAppApi00701Result();
...
//				}
				while (line != null) {
					HulunbuirAppApi00701Result aa00201Result = new HulunbuirAppApi00701Result();
					try {
...
//							if(sum>num){
								...
								aa00201Result.setTransdate(param[1].trim());
								aa00201Result.setLoanfundtype(param[2].trim());
								aa00201Result.setSummarycode(param[3].trim());
								aa00201Result.setLoanamt(param[4].trim());
								aa00201Result.setCurbal(param[5].trim());
...
//						sum+=1;
		...
		for(int i=0;i<list.size();i++){
			appapi00101Result = JavaBeanUtil.javaBeanToListTitleInfoBean("appapi00701"+form.getCenterId()+".result", list.get(i));			
