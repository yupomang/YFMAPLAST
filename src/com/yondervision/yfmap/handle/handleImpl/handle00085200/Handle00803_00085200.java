...
/** 
* @ClassName: Handle00803_00085200 
* @Description: 遵义栋号楼盘查询
* @author Caozhongyan
* @date Apr 8, 2016 4:57:14 PM   
* 
*/ 
public class Handle00803_00085200 implements CtrlHandleInter {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap) throws CenterRuntimeException,Exception{
		...
		log.debug("YFMAP 可贷栋号楼盘查询 form 参数:"+form);		
		...
		Date date = new Date();
		String fname = formatter.format(date)+"api00803LPDHGJJ.txt";
		if(Constants.method_BSP.equals(send)){
			...
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REQ_KDLPDHCX.txt", map, req);
...
//			String rexml = "<AuthCode1>IVR0</><AuthCode2>IVR0</><AuthCode3>IVR0</><AuthFlag>0</><BrcCode>00000000</><BusiSeq>90</><ChannelSeq>398243</><ChkCode>cweb</><MTimeStamp>2013-07-24 09:55:28:967000</><NoteMsg></><RspCode>000000</><RspMsg>成功</><STimeStamp>2015-07-24 09:55:28:966000</><TellCode>cweb</><TranChannel>00</><TranCode>116005</><TranDate>2016-04-06</><TranIP>10.22.21.26</><TranSeq>90</><fileName>lpdh.txt</><sumcounts></><counts></><pageflag>1</>";
			...
			if(!app00802Result.getFileName().equals("")){
				...
				int projectname=0,cocustname=0,housesit=0,polename=0;
				for(int i=0;i<paraNames.length;i++){
					if("projectname".equals(paraNames[i])){
						...
						housesit = i;//楼盘地址
					}else if("polename".equals(paraNames[i])){
						polename = i;//栋号
					}
				}
...
//					return modelMap;
				...
				while (line != null) {					
					try {
						...
						mi203.setTel("");//电话
						mi203.setContacterName("");//联系人
						mi203.setAddress(param[housesit]);//地址
						mi203.setHouseName(param[projectname]);//楼盘名
						mi203.setDeveloperName(param[cocustname]);//开发商名
						mi203.setBankNames(param[polename]);
						AppApi00801Result01 appApi00801Result01 = new AppApi00801Result01();
						appApi00801Result01.setX(param[projectname]);//栋号
						appApi00801Result01.setY(param[polename]);//栋号						
						appApi00801Result01.setImg(param[polename]);//栋号
						appApi00802Result.setMap(appApi00801Result01);
						TitleInfoBeanContent = JavaBeanUtil.javaBeanToListTitleInfoBean("appapi00803"+form.getCenterId()+".content", mi203);													
						appApi00802Result.setContent(TitleInfoBeanContent);
						TitleInfoBeanList = JavaBeanUtil.javaBeanToListTitleInfoBean("appapi00803"+form.getCenterId()+".list", mi203);
	...
	public static void main(String[] args){
		...
		form1.setPagenum("10");
		form1.setPagerows("0");
		Handle00803_00085200 hand = new Handle00803_00085200();
		try {
