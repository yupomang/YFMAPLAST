...
import com.yondervision.yfmap.result.hulunbuir.HulunbuirAppApi0100601Result;
import com.yondervision.yfmap.result.hulunbuir.HulunbuirAppApi0100801Result;
...
/** 
* @ClassName: Handle0100601_00047000 
* @Description: 销户提取
* 
*/ 
public class Handle0100601_00047000 implements CtrlHandleInter {
	Logger log = Logger.getLogger("YFMAP");
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap) throws CenterRuntimeException,Exception{
		log.info(Constants.LOG_HEAD+"appApi0100601 start.");
		...
		HulunbuirAppApi0100601Result app40101Result = new HulunbuirAppApi0100601Result();
		HulunbuirAppApi0100601Result app40101Result_bank = new HulunbuirAppApi0100601Result();
		if(Constants.method_BSP.equals(send)){////xml通信处理
			...
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form1.getCenterId())+"/BSP_REQ_TQXXCX"+form.getDrawSignType()+".txt", map, req);
			log.debug("前置YFMAP发往中心报文："+xml);
			
			String ip = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_IP"+form.getCenterId()).trim();
			String port = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_PORT"+form.getCenterId()).trim();
			String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "159004");
...
//			String rexml = "<AuthCode1>cweb</><AuthCode2>cweb</><AuthCode3>cweb</><AuthFlag>0</><BrcCode>12345678</><BusiSeq>1269</><ChannelSeq>398243</><ChkCode>cweb</><MTimeStamp>2016-08-10 05:04:09</><NoteMsg></><RspCode>000000</><RspMsg>交易处理成功...</><STimeStamp>2016-08-10 05:04:09</><TellCode>cweb</><TranChannel>00</><TranCode>112813</><TranDate>2016-08-10</><TranIP>192.168.10.20</><TranSeq>1269</><accname>刘文龙</><accname1>正常</><accnum>801003368857</><bal>10366.79</><basenum>2145.60</><certinum>220122199107225011</><indipayamt>257.47</><indipaysum>514.94</><indiprop>0.120</><lasttransdate>2016-07-01</><lpaym>201604</><opnaccdate>2014-07-01</><unitaccname>乌兰察布电业局</><unitaccnum>01030032</><unitpayamt>257.47</><unitprop>0.120</>";
			...
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form1.getCenterId())+"/BSP_REP_TQXXCX"+form.getDrawSignType()+".txt", rexml, req);
			...
			xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form1.getCenterId())+"/BSP_REQ_YHLBCX.txt", map, req);
			log.debug("前置YFMAP发往中心报文："+xml);
			
			rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "149367");
			
//			String rexml = "<AuthCode1>cweb</><AuthCode2>cweb</><AuthCode3>cweb</><AuthFlag>0</><BrcCode>12345678</><BusiSeq>1269</><ChannelSeq>398243</><ChkCode>cweb</><MTimeStamp>2016-08-10 05:04:09</><NoteMsg></><RspCode>000000</><RspMsg>交易处理成功...</><STimeStamp>2016-08-10 05:04:09</><TellCode>cweb</><TranChannel>00</><TranCode>112813</><TranDate>2016-08-10</><TranIP>192.168.10.20</><TranSeq>1269</><accname>刘文龙</><accname1>正常</><accnum>801003368857</><bal>10366.79</><basenum>2145.60</><certinum>220122199107225011</><indipayamt>257.47</><indipaysum>514.94</><indiprop>0.120</><lasttransdate>2016-07-01</><lpaym>201604</><opnaccdate>2014-07-01</><unitaccname>乌兰察布电业局</><unitaccnum>01030032</><unitpayamt>257.47</><unitprop>0.120</>";
			//String rexml = "<AuthCode1>9003</><AuthCode2>9003</><AuthCode3>9003</><AuthFlag></><BrcCode>00003102</><BusiSeq>171</><ChannelSeq>695491</><ChkCode>9003</><FinancialDate>2016-07-01</><HostBank></><MTimeStamp>2016-11-15 12:21:46</><NoteMsg></><RspCode>000000</><RspMsg>交易处理成功...</><STimeStamp>2016-12-09 11:26:37</><SubBank></><TellCode>9003</><TranChannel>01</><TranCode>311602</><TranDate>2016-11-21</><TranIP>18.16.2.9</><TranSeq>171</><accname>李硕欣</><accnum>104302</><balance>61097.88</><bankname>中行包头分行营业部</><basenumber>5436.00</><cardno>6013828402007533909</><certinum>150203196809222768</><indiprop>12.00</><lastpaydate>2016年04月</><monpaysum>1306.00</><peraccstate>正常</><peryje>653.00</><prehapdate>2016年07月01日</><prop>24.00</><unitaccname>包钢集团电气有限公司</><unitaccnum>2112</><unitprop>12.00</><unityje>653.00</>";
			HashMap resultMap_bank = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form1.getCenterId())+"/BSP_REP_YHLBCX.txt", rexml, req);
			log.debug("YFMAP接收解析报文MAP："+resultMap_bank);
			BeanUtil.transMap2Bean(resultMap_bank, app40101Result_bank);
			log.debug("MAP封装成BEAN："+app40101Result_bank);
			if(!Constants.sucess_ts.equals(app40101Result_bank.getRecode())){
				modelMap.put("recode", app40101Result_bank.getRecode());
				modelMap.put("msg", app40101Result_bank.getMsg());
				log.error("中心返回报文 状态recode :"+app40101Result_bank.getRecode()+"  ,  描述msg : "+app40101Result_bank.getMsg());
				return modelMap;
			}
		}
		List<TitleInfoBean> banklist = new ArrayList<TitleInfoBean>();
		String yhbm_temp[] = app40101Result_bank.getBankstring().split(",");
		for(int i=0;i<yhbm_temp.length;i++)
		{
			TitleInfoBean t = new TitleInfoBean();
			String yhbm_t[] = yhbm_temp[i].split(":");
			if(yhbm_t.length>=0)t.setTitle(yhbm_t[0].replaceAll("\"", ""));
			if(yhbm_t.length>=1)t.setInfo(yhbm_t[1].replaceAll("\"", ""));
			banklist.add(t);
		}
		...
		List<TitleInfoBean> appapi40101Result = JavaBeanUtil.javaBeanToListTitleInfoBean("appapi01006"+form.getCenterId()+".result"+form.getDrawSignType(), app40101Result);
		modelMap.clear();
		modelMap.put("recode", "000000");
		modelMap.put("msg", "成功");
		modelMap.put("bal", app40101Result.getBal());
		modelMap.put("sealtype", app40101Result.getSealtype());
		modelMap.put("unitaccnum", app40101Result.getUnitaccnum());
