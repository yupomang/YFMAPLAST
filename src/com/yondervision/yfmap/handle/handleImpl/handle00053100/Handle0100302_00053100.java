 ...
 * 提取信息查询-委托提取签约
 */
public class Handle0100302_00053100 implements CtrlHandleInter {
...
//				return modelMap;
//			}
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"REQ_TQYYCX02.xml", map, req);
...
//					"</body></root>";
			...
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"REP_TQYYCX02.xml", rexml, req);
//			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, "D:/server/apache-tomcat-6.0.44/webapps/ydmi_csp/WEB-INF/classes/com/yondervision/yfmap/config/messagetemp/00053100/REP_TQYYCX02.xml", rexml, req);
		...
		modelMap.put("bankIdcard", aes.encrypt(app01003Result.getBankIdcard().replaceAll("--", "").getBytes("UTF-8")));
		modelMap.put("bankNumber", aes.encrypt(app01003Result.getBankNumber().replaceAll("--", "").getBytes("UTF-8")));
		modelMap.put("loanflag", app01003Result.getLoanflag().replaceAll("--", ""));
		modelMap.put("relphone", app01003Result.getPhoneNumber().replaceAll("--", ""));
		modelMap.put("loannum", aes.encrypt(app01003Result.getLoannum().replaceAll("--", "").getBytes("UTF-8")));
		modelMap.put("signStatus", app01003Result.getSignStatus());
		modelMap.put("drawSignType", app01003Result.getDrawSignType());
		modelMap.put("result", new JSONObject());
	...
	public static void main(String[] args){
		...
		Handle0100302_00053100 hand = new Handle0100302_00053100();
		try {
