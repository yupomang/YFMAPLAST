...
public class Handle07025_00057400 implements CtrlHandleInter {
...
//				form.setBrcCode(PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "appapi001BrcCode"+form.getCenterId()).trim());
		    ...
		    String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REQ_07025_128112.txt", map, req);
			log.debug("前置YFMAP发往中心报文："+xml);
			String ip = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_IP"+form.getCenterId()).trim();
			String port = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_PORT"+form.getCenterId()).trim();
			String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "128112");
			log.debug("前置YFMAP接收中心报文——提前还款确认金额："+rexml);
		    HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REP_07025_128112.txt", rexml, req);
		...
		modelMap.put("msg", "成功");
		modelMap.put("repayint", app07010ZHResult.getRepayint());
		modelMap.put("repayprin", app07010ZHResult.getRepayprin());
		modelMap.put("repaypun", app07010ZHResult.getRepaypun());
		return modelMap;
	}
}
