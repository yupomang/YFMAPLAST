...
import com.yondervision.yfmap.result.haikou.AppApi01029Result;
 ...
 *
 */
public class Handle01029_00089800 implements CtrlHandleInter {
	
	Logger log = Logger.getLogger("YFMAP");
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap) throws CenterRuntimeException,Exception{
		String PROPERTIES_FILE_NAME = "properties.properties";
		AppApi01028Form form = (AppApi01028Form)form1;
		log.info(Constants.LOG_HEAD+"appApi01029 start.");
		...
		AppApi01029Result app01029Result = new AppApi01029Result();
		...
		if(Constants.method_BSP.equals(send)){
			...
			/* 第1次通信   查询提取相关信息*/
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REQ_YWBLHK129129.txt", map, req);
			log.debug("发往中心报文-相关信息获取："+xml);
			
			// 通信
			String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "129129");
			log.debug("中心返回报文-相关信息获取："+rexml);
			
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType,CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"BSP_REP_YWBLHK129129.txt", rexml, req);
			...
			BeanUtil.transMap2Bean(resultMap, app01029Result);
			
			// isloanflag 根据笔数判断是否签订贷款 .  useflag>0,isloanflag=1 是		useflag<=0,isloanflag=0 否	
			try{
				int useflag = Integer.parseInt(app01029Result.getCounts());
				if(useflag > 0){
					app01029Result.setIsloanflag("1");
				} else {
					app01029Result.setIsloanflag("0");
				}
			} catch(Exception e){
				
				modelMap.clear();
				modelMap.put("recode", "999999");
				modelMap.put("msg", "查询笔数不是数字。无法判断。");
				log.error("中心返回报文 状态recode :999999  ,  描述msg : 查询笔数不是数字。无法判断。" + e);
				return modelMap;
			}
		}
		
		List<TitleInfoBean> appapi00101Result = null;
		appapi00101Result = JavaBeanUtil.javaBeanToListTitleInfoBean("appapi01029"+form.getCenterId()+".result", app01029Result);
		...
		modelMap.put("result1", app01029Result);
