...
import com.yondervision.yfmap.result.ningbo.AppApi5002501ZHResult;
import com.yondervision.yfmap.result.ningbo.AppApi5002502ZHResult;
import com.yondervision.yfmap.result.ningbo.AppApi5002503ZHResult;
...
* 
*/ 
public class Handle50025_00057400 implements CtrlHandleInter1 {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap,HttpServletRequest request,HttpServletResponse response) throws CenterRuntimeException,Exception{
		...
		System.out.println("YFMAP发往中心——资统业务统计分析查询");
		...
		if(Constants.method_BSP.equals(send)){
			...
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REQ_50025_143011.txt", map, req);
			log.debug("前置YFMAP发往中心报文："+xml);
			String ip = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_IP"+form.getCenterId()).trim();
			String port = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_PORT"+form.getCenterId()).trim();
			String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "143011");			
			log.debug("前置YFMAP接收中心报文："+rexml);
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REP_50025_143011.txt", rexml, req);
