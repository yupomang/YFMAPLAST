...
public class Handle07022_00057400 implements CtrlHandleInter {
...
//				form.setBrcCode(PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "appapi001BrcCode"+form.getCenterId()).trim());
			...
			byte[] d = DESForJava.encryption(form.getNewpassword(),"12345678");
			byte[] e = DESForJava.encryption(form.getConfirmnewpassword(),"12345678");
			c = DESForJava.bcd_to_asc(c);
			d = DESForJava.bcd_to_asc(d);
			e = DESForJava.bcd_to_asc(e);
			form.setPassword(new String(c));
			form.setNewpassword(new String(d));
			form.setConfirmnewpassword(new String(e));

			if("0".equals(form.getInfotype())){
				form.setBrcCode("05740008");
			}
			
			HashMap map = BeanUtil.transBean2Map(form);		
			map.put("tellcode", PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "appapichannel"+form.getChannel()).trim());
		    String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REQ_07022_128185.txt", map, req);
			log.debug("前置YFMAP发往中心报文："+xml);
			String ip = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_IP"+form.getCenterId()).trim();
			String port = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_PORT"+form.getCenterId()).trim();
			String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "128185");
			log.debug("前置YFMAP接收中心报文——开发商密码修改与重置："+rexml);
		    HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REP_07022_128185.txt", rexml, req);
