...
public class Handle3004_00047000 extends AbsBusinessServer {
	...
	public String acction(String xml, OutputStream out, String centerid, String seq)
		...
		centerId = centerid;
		log.debug("Handle3004_00047000:开始****处理模板信息单笔推送逻辑");
		...
		if(Constants.method_XML.equals(send)){////xml通信处理
			...
			try {
				log.debug("Handle3004_00047000 接收报文: "+xml);
			...
			log.debug("Handle3004_00053100:acction封装移动平台JSON数据："+param);
...
//			resultMap.put("recvTime",appapi3004result.getSendSeqno());
			...
			try {
				remsg = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+centerid)+"REP_HX_MB_MESSAGE_RETURN.xml", resultMap, seq);				
				log.debug("Handle3004_00053100:acction返回报文："+remsg);
				System.out.println("Handle3004_00053100:acction返回报文："+remsg);
			} catch (IOException e) {				
				e.printStackTrace();
		...
		log.debug("Handle3004_00053100:结束****处理模板信息单笔推送逻辑");
		return remsg;
	}
	...
	private void sendUrl() {
		...
		log.debug("Handle3004_00053100:YBMAP url ："+url);
		System.out.println("Handle3004_00053100:YBMAP url ："+url);
		resultMap.put("centerId", centerId);
		String rm1 = post.post(url, resultMap);
		System.out.println("Handle3004_00053100:YBMAP 返回信息l ："+rm1);
		Gson gson1 = new Gson();
	...
	private void sendUrl1020() {
		...
		log.debug("Handle3004_00053100:YBMAP url ："+url1);
		System.out.println("Handle3004_00053100:YBMAP url ："+url1);
		resultMap.put("centerId", centerId);
		...
		System.out.println("Handle3004_00053100:YBMAP 返回信息l ："+rm);
		Gson gson = new Gson();
