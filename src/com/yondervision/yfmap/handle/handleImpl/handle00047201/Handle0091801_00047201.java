...
import com.yondervision.yfmap.result.baogang.BaoGangAppApi00918Result;
...
/** 
* @ClassName: Handle0091801_00047201 
* @Description: 贷后变更试算——提前部分还款
* 
*/ 
public class Handle0091801_00047201 implements CtrlHandleInter {
	Logger log = Logger.getLogger("YFMAP");
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap) throws CenterRuntimeException,Exception{
		log.info(Constants.LOG_HEAD+"appApi0091801 start.");
		String PROPERTIES_FILE_NAME = "properties.properties";
		AppApi00918Form form = (AppApi00918Form)form1;
		AES aes =new AES();
		form.setIdcardNumber(aes.decrypt(form.getIdcardNumber()));
		log.debug("贷后变更试算——提前部分还款请求参:"+form);
		String send = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "appapi001Send"+form.getCenterId()).trim();
		BaoGangAppApi00918Result app40101Result = new BaoGangAppApi00918Result();
		if(Constants.method_BSP.equals(send)){////xml通信处理
			...
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form1.getCenterId())+"/BSP_REQ_DHBGSS.txt", map, req);
			log.debug("前置YFMAP发往中心报文："+xml);
			
			String ip = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_IP"+form.getCenterId()).trim();
			String port = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_PORT"+form.getCenterId()).trim();
			String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "312445");
...
//			String rexml = "<AuthCode1>cweb</><AuthCode2>cweb</><AuthCode3>cweb</><AuthFlag>0</><BrcCode>12345678</><BusiSeq>1269</><ChannelSeq>398243</><ChkCode>cweb</><MTimeStamp>2016-08-10 05:04:09</><NoteMsg></><RspCode>000000</><RspMsg>交易处理成功...</><STimeStamp>2016-08-10 05:04:09</><TellCode>cweb</><TranChannel>00</><TranCode>112813</><TranDate>2016-08-10</><TranIP>192.168.10.20</><TranSeq>1269</><accname>刘文龙</><accname1>正常</><accnum>801003368857</><bal>10366.79</><basenum>2145.60</><certinum>220122199107225011</><indipayamt>257.47</><indipaysum>514.94</><indiprop>0.120</><lasttransdate>2016-07-01</><lpaym>201604</><opnaccdate>2014-07-01</><unitaccname>乌兰察布电业局</><unitaccnum>01030032</><unitpayamt>257.47</><unitprop>0.120</>";
			...
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form1.getCenterId())+"/BSP_REP_DHBGSS.txt", rexml, req);
		...
		List<TitleInfoBean> appapi40101Result = JavaBeanUtil.javaBeanToListTitleInfoBean("appapi0091801"+form.getCenterId()+".result", app40101Result);
		log.debug("YFMAPappapi40101Result："+appapi40101Result);
		modelMap.clear();
		modelMap.put("recode", "000000");
		modelMap.put("msg", "成功");
		modelMap.put("result", appapi40101Result);
		log.info(Constants.LOG_HEAD+"appApi0091801 end.");
		return modelMap;
	}
	public static void main(String[] args) {
		String req = "";
		String rexml = "<AuthCode1>9003</><AuthCode2>9003</><AuthCode3>9003</><AuthFlag></><BrcCode>00003102</><BusiSeq>1201</><ChannelSeq>695491</><ChkCode>9003</><FinancialDate>2016-07-01</><HostBank></><MTimeStamp>2016-11-15 12:21:46</><NoteMsg></><RspCode>000000</><RspMsg>交易处理成功...</><STimeStamp>2016-12-15 11:03:08</><SubBank></><TellCode>9003</><TranChannel>01</><TranCode>312445</><TranDate>2016-11-21</><TranIP>18.16.2.9</><TranSeq>1201</><ccurdaybal>129097.78</><ctrlamt>37983.04</><dInt1>18496.03</><lloanintbal>24898.95</><loanint>16543.82</><loanintbal>25343.52</><newint>444.57</><newmonth>1768.00</><nmonthpay>0.00</><nowprin>179080.00</><oweamt>0.00</><oweint>0.00</><oweprin>0.00</><paymont>2271.37</><pmonthpay>1618.24</><pun>0.00</><rate>0.000000</><tolamt>179524.57</><wnewint>31114.40</>";
		HashMap resultMap = MessageCtrMain.analysisPacket("5", "D:/tomcat/webapps/ydmi_csp/WEB-INF/classes/com/yondervision/yfmap/config/messagetemp/00047201/BSP_REP_DHBGSS.txt", rexml, req);
		BaoGangAppApi00918Result app40101Result = new BaoGangAppApi00918Result();
		BeanUtil.transMap2Bean(resultMap, app40101Result);
		List<TitleInfoBean> appapi40101Result = JavaBeanUtil.javaBeanToListTitleInfoBean("appapi009180100047201.result", app40101Result);
		System.out.println("YFMAPappapi40101Result："+appapi40101Result);
	}
}
