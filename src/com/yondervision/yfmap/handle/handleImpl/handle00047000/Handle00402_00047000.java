...
import com.yondervision.yfmap.result.hulunbuir.HulunbuirAppApi00201Result;
...
public class Handle00402_00047000 implements CtrlHandleInter {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap) throws CenterRuntimeException,Exception{
		/* 模拟返回开始  */	
		log.info(Constants.LOG_HEAD+"appApi00402 start.");
		AppApi00401Form form = (AppApi00401Form)form1;
		List<HulunbuirAppApi00402Result> list = new ArrayList<HulunbuirAppApi00402Result>();
		...
		String fname = formatter2.format(date)+"_"+form.getSurplusAccount()+"_"+"api00201GJJMX.txt";
		HulunbuirAppApi00402Result app002Result = new HulunbuirAppApi00402Result();
		if(Constants.method_BSP.equals(send)){////xml通信处理
			...
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REQ_GRZLBGCX.txt", map, req);
			log.debug("YFMAP发往中心报文："+xml);
				
			String ip = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_IP"+form.getCenterId()).trim();
			String port = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_PORT"+form.getCenterId()).trim();
			String reqxml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "111053");
...
//			String reqxml = "<AuthCode1>IVR0</><AuthCode2>IVR0</><AuthCode3>IVR0</><AuthFlag>0</><BrcCode>00000000</><BusiSeq>90</><ChannelSeq>398243</><ChkCode>cweb</><MTimeStamp>2013-07-24 09:55:28:967000</><NoteMsg></><RspCode>000000</><RspMsg>成功</><STimeStamp>2015-07-24 09:55:28:966000</><TellCode>cweb</><TranChannel>00</><TranCode>116005</><TranDate>2016-04-06</><TranIP>10.22.21.26</><TranSeq>90</><fileName>YEMXCX.txt</><accnum></><accname></><sumcounts></><counts></><pageflag>1</>";
			...
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/BSP_REP_GRZLBGCX.txt", reqxml, req);
...
//				}
				while (line != null) {
					HulunbuirAppApi00402Result aa00201Result = new HulunbuirAppApi00402Result();
					try {
//						if(pnum<Integer.valueOf(form.getPagerows())){
//							if(sum>num){
								String[] param = line.split("~");									
								log.debug("读取文件  ："+line);
								aa00201Result.setAccnum(param[0].trim());	
								aa00201Result.setAccname(param[1].trim());
								aa00201Result.setCustchgtype(param[2].trim());
								aa00201Result.setDesc(param[3].trim());
								aa00201Result.setBfchgval(param[4].trim());
								aa00201Result.setAfchgval(param[5].trim());
								aa00201Result.setTransdate(param[6].trim());
								aa00201Result.setAgentinstcode(param[7].trim());
								//aa00201Result.setAgentopname(param[8].trim());
...
//						sum+=1;
		...
		for(int i=0;i<list.size();i++){
			appapi00101Result = JavaBeanUtil.javaBeanToListTitleInfoBean("appapi00402"+form.getCenterId()+".result", list.get(i));			
