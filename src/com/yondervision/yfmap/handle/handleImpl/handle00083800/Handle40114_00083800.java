...
* 
*/ 
public class Handle40114_00083800 implements CtrlHandleInter {
...
//		form.setNewpassword(aes.decrypt(form.getNewpassword()));
		byte[] b = DESForJava_CD.encryption(aes.decrypt(form.getPassword()), "12345678");
		byte[] c = DESForJava_CD.bcd_to_asc(b);
		form.setPassword(new String(c));
		byte[] d = DESForJava_CD.encryption(aes.decrypt(form.getNewpassword()), "12345678");
		byte[] e = DESForJava_CD.bcd_to_asc(d);
		form.setNewpassword(new String(e));
		...
		if(Constants.method_BSP.equals(send)){////xml通信处理
			...
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/REQ_MMXG.txt", map, req);
			log.debug("YFMAP发往中心报文："+xml);
			
			String ip = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_IP"+form.getCenterId()).trim();
			String port = PropertiesReader.getProperty(PROPERTIES_FILE_NAME, "BSP_SOCKET_PORT"+form.getCenterId()).trim();
			String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "430008");
...
//			String rexml = "<AuthCode1>IVR0</><AuthCode2>IVR0</><AuthCode3>IVR0</><AuthFlag>0</><BrcCode>00000000</><BusiSeq>90</><ChannelSeq>398243</><ChkCode>cweb</><MTimeStamp>2013-07-24 09:55:28:967000</><NoteMsg></><RspCode>000000</><RspMsg>成功</><STimeStamp>2013-07-24 09:55:28:966000</><TellCode>cweb</><TranChannel>00</><TranCode>116004</><TranDate>2014-12-11</><TranIP>10.22.21.26</><TranSeq>90</><fileName></><pubaccnum>123456</><accname>甲六</>";
			...
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"/REP_MMXG.txt", rexml, req);
