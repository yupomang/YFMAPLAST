 ...
 *
 */
public class Handle011_00045101 implements CtrlHandleInter {
...
//			String xml = MessageCtrMain.encapsulatedPackets(msgType, "E:/peixun/YFMAP/WebRoot/WEB-INF/classes/com/yondervision/yfmap/config/messagetemp/00045101"+"/BSP_REQ_DKJDCX.txt", map, req);//CommonUtil.getFullURL(Constants.msgPath+"00045101")
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+"00045101")+"/BSP_REQ_DKJDCX.txt", map, req);
			...
			String reqxml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "120325");
			
//			String reqxml = "<AuthCode1>0000</><AuthCode2>0000</><AuthCode3>0000</><AuthFlag>0</><BrcCode>00000000</><BusiSeq>2645035</><ChannelSeq>398243</><ChkCode>0000</><MTimeStamp>2015-12-16 11:23:14</><NoteMsg></><RspCode>000000</><RspMsg>交易处理成功...</><STimeStamp>2015-12-16 11:23:14</><TellCode>0000</><TranChannel>00</><TranCode>120325</><TranDate>2015-12-16</><TranIP>10.22.21.26</><TranSeq>2645035</><filename>ddd.txt</>";
			log.debug("CSP返回YFMAP报文："+reqxml);
			
			
//			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, "E:/peixun/YFMAP/WebRoot/WEB-INF/classes/com/yondervision/yfmap/config/messagetemp/00045101"+"/BSP_REP_DKJDCX.txt", reqxml, req);//CommonUtil.getFullURL(Constants.msgPath+"00045101")
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+"00045101")+"/BSP_REP_DKJDCX.txt", reqxml, req);
...
//						app007Result.setDesstate(param[9]);//交易状态						
		...
		for(int i=0;i<list.size();i++){
			...
			result1 = JavaBeanUtil.javaBeanToListTitleInfoBean("appapi01101"+form.getCenterId()+".detail", (NongKenAppApi00701Result)list.get(i));	
			result.add(result1);
		}
...
//		Handle011_00045101 hand = new Handle011_00045101();
//		try {
