...
import com.yondervision.yfmap.result.tangshan.TangShanAppApi01008Result;
 ...
 * 变更登记查询-提前还款登记
 */
public class Handle0100803_00031500 implements CtrlHandleInter {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap)
		...
		log.debug("form:"+form); 
		/* 模拟返回开始  */
		TangShanAppApi01008Result app01006Result= new TangShanAppApi01008Result();
...
//				return modelMap;
//			}
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"REQ_BGDJCX03.xml", map, req);
...
//			String rexml = "<?xml version=\"1.0\" encoding = \"UTF-8\"?><root><head><reqflag>1</reqflag><msgtype>75</msgtype><tr_code>X00005</tr_code><corp_no>00053100</corp_no><oreq_no>8020150929007169</oreq_no><tr_acdt>2015-09-29</tr_acdt><tr_time>10:25:15</tr_time><ans_code>000000</ans_code><ans_info>交易成功完成</ans_info><particular_code>000000</particular_code><particular_info>交易成功完成</particular_info><reserved></reserved></head><body><xm>夏洛</xm><zjlx>身份证</zjlx><zjhm>220001197701011234</zjhm><gfdz>梧桐花园88号</gfdz><dkje>500000.00</dkje><nx>30</nx><hkfs>等额本金</hkfs><dkye>400000.00</dkye><zkyh>建行</zkyh><yhhkzh>2012748300917887</yhhkzh><htdqrq>2025-12-31</htdqrq><cqzbz>未入库</cqzbz><czrq>2015-01-01</czrq></body></root>";
			...
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"REP_BGDJCX03.xml", rexml, req);
//			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, "D:/server/apache-tomcat-6.0.44/webapps/ydmi_csp/WEB-INF/classes/com/yondervision/yfmap/config/messagetemp/00053100/REP_TQYYCX02.xml", rexml, req);
			...
			BeanUtil.transMap2Bean(resultMap, app01006Result);
			
			
			if(!Constants.sucess_ts.equals(app01006Result.getRecode())){
				modelMap.put("recode", app01006Result.getRecode());
				modelMap.put("msg", app01006Result.getMsg());
				log.error("中心返回报文 状态recode :"+app01006Result.getRecode()+"  ,  描述msg : "+app01006Result.getMsg());
				return modelMap;
			}
		}		
		app01006Result.setQb(String.format("%.2f",Double.valueOf(app01006Result.getQb()))+"元");
		app01006Result.setQx(String.format("%.2f",Double.valueOf(app01006Result.getQx()))+"元");
		app01006Result.setQfx(String.format("%.2f",Double.valueOf(app01006Result.getQfx()))+"元");
		app01006Result.setQkhj(String.format("%.2f",Double.valueOf(app01006Result.getQkhj()))+"元");
		app01006Result.setWdqbj(String.format("%.2f",Double.valueOf(app01006Result.getWdqbj()))+"元");
		app01006Result.setXcslx(String.format("%.2f",Double.valueOf(app01006Result.getXcslx()))+"元");
		app01006Result.setTqqbhkje(String.format("%.2f",Double.valueOf(app01006Result.getTqqbhkje()))+"元");
		...
		List<TitleInfoBean> appapi01008Result = JavaBeanUtil.javaBeanToListTitleInfoBean("appapi0100803"+form.getCenterId()+".result", app01006Result);
		
		modelMap.clear();
		modelMap.put("recode", "000000");
		modelMap.put("msg", "成功");
		modelMap.put("result", appapi01008Result);
	...
	public static void main(String[] args){
		...
		Handle0100803_00031500 hand = new Handle0100803_00031500();
		try {
