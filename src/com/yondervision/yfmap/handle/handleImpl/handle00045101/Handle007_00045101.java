 ...
 *
 */
public class Handle007_00045101 implements CtrlHandleInter {
...
//			String xml = MessageCtrMain.encapsulatedPackets(msgType, "E:/peixun/YFMAP/WebRoot/WEB-INF/classes/com/yondervision/yfmap/config/messagetemp/00045101"+"/BSP_REQ_DKMXCX.txt", map, req);//CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())
...
//			String reqxml="<AuthCode1>0000</><AuthCode2>0000</><AuthCode3>0000</><AuthFlag>0</><BrcCode>00000000</><BusiSeq>2644864</><ChannelSeq>398243</><ChkCode>0000</><MTimeStamp>11:21:09</><NoteMsg></><RspCode>000000</><RspMsg>交易处理成功...</><STimeStamp>11:21:09</><TellCode>0000</><TranChannel>00</><TranCode>120323</><TranDate>2015-12-16</><TranIP>10.22.21.26</><TranSeq>2644864</><filename>eee.txt</>";
			log.debug("CSP返回YFMAP报文："+reqxml);
			
			
			
//			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, "E:/peixun/YFMAP/WebRoot/WEB-INF/classes/com/yondervision/yfmap/config/messagetemp/00045101"+"/BSP_REP_DKMXCX.txt", reqxml, req);//CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())
...
//						}
						...
						app007Result.setTransdate(param[1]);//交易日期
						app007Result.setHostsernum(param[2]);//流水号
						app007Result.setSummary(param[3]);//业务类型
						app007Result.setRepaytolamt(String.format("%,.2f",Double.valueOf(param[4])));//偿还总金额
						app007Result.setRepayprin(String.format("%,.2f",Double.valueOf(param[5])));//偿还本金
						app007Result.setRepayint(String.format("%,.2f",Double.valueOf(param[6])));//偿还利息
						app007Result.setRepaypun(String.format("%,.2f",Double.valueOf(param[7])));//偿还罚息
						app007Result.setLoanbal(String.format("%,.2f",Double.valueOf(param[8])));//账户余额
		...
		log.debug("appapi00701"+form.getCenterId()+".detail");
		...
		for(int i=0;i<list.size();i++){
			...
			result1 = JavaBeanUtil.javaBeanToListTitleInfoBean("appapi00701"+form.getCenterId()+".detail", (NongKenAppApi00701Result)list.get(i));	
			result.add(result1);
		}
...
//		Handle007_00045101 hand = new Handle007_00045101();
//		try {
