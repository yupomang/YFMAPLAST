 ...
 * 个人账户查询
 */
public class Handle40121_00053100 implements CtrlHandleInter {
...
//				return modelMap;
//			}
			String xml = MessageCtrMain.encapsulatedPackets(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"REQ_GRZHCX.xml", map, req);
//			String xml = MessageCtrMain.encapsulatedPackets(msgType, "D:/server/apache-tomcat-6.0.44/webapps/ydmi_csp/WEB-INF/classes/com/yondervision/yfmap/config/messagetemp/00053100/REQ_GRZHCX.xml", map, req);
			...
			String rexml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "33000022");
...
//					"<body>		" +
//					"<certinum>"+form.getIdcardNumber()+"</certinum>" +
//					"<accnum>123123123</accnum>" +
//					"</body></root>";

			
			log.debug("中心下传报文："+rexml);
			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, CommonUtil.getFullURL(Constants.msgPath+form.getCenterId())+"REP_GRZHCX.xml", rexml, req);
//			HashMap resultMap = MessageCtrMain.analysisPacket(msgType, "D:/server/apache-tomcat-6.0.44/webapps/ydmi_csp/WEB-INF/classes/com/yondervision/yfmap/config/messagetemp/00053100/REP_GRZHCX.xml", rexml, req);
		...
		if(CommonUtil.isEmpty(app40119Result.getIdcardNumber()))
		{
			obj.put("idcardNumber",aes.encrypt("".getBytes("UTF-8")));
		}else
		{
			obj.put("idcardNumber",aes.encrypt(app40119Result.getIdcardNumber().getBytes("UTF-8")));
		}
		modelMap.clear();
	...
	public static void main(String[] args){
		...
		Handle40121_00053100 hand = new Handle40121_00053100();
		try {
