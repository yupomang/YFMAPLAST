...
public class Handle00802_00031500 implements CtrlHandleInter {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap) throws CenterRuntimeException,Exception{
		...
		List<TangShanAppApi00802_2Result> list = new ArrayList<TangShanAppApi00802_2Result>();
		AES aes = new AES();
...
//			HashMap resultMap = MessageCtrMain.analysisPacket(msgType,"C:/Program Files/Tomcat 6.0/webapps/YFMAP/WEB-INF/classes/com/yondervision/yfmap/config/messagetemp/00053100/REP_KDLPCX.xml", rexml, req);
			...
			if(filename!=null){
				...
				while (line != null) {// 判断读取到的字符串是否不为空
					TangShanAppApi00802_2Result aa00802Result = new TangShanAppApi00802_2Result();
					try {
						String[] valus = line.split("\\@\\|\\$");
						log.debug("读取文件  ："+line);
						if(valus.length>0)aa00802Result.setLpmc(valus[0]);
						if(valus.length>1)aa00802Result.setWdmc(valus[1]);
						if(valus.length>2)aa00802Result.setKfdw(valus[2]);
						if(valus.length>3)aa00802Result.setPzzh(valus[3]);
...
//		System.out.println(JsonUtil.getGson().toJson(modelMap));
	...
	public static void main(String[] args){
		...
		form1.setPagenum("10");
		form1.setPagerows("0");
		Handle00802_00031500 hand = new Handle00802_00031500();
		try {
