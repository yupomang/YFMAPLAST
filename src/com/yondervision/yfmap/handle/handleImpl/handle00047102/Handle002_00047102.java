...
import com.yondervision.yfmap.result.qvzhi.QvZhiAppApi002Result;
...
* 
*/ 
public class Handle002_00047102 implements CtrlHandleInter {
	...
	public ModelMap action(AppApiCommonForm form1, ModelMap modelMap) throws CenterRuntimeException,Exception{
		...
		List<QvZhiAppApi002Result> list = new ArrayList<QvZhiAppApi002Result>();
		...
		if(Constants.method_BSP.equals(send)){////xml通信处理
			...
			String reqxml = CommunicateUtil.sendMessage(send, ip, Integer.parseInt(port), xml, "119128");
...
//			String reqxml = "<AuthCode1>IVR0</><AuthCode2>IVR0</><AuthCode3>IVR0</><AuthFlag>0</><BrcCode>00000000</><BusiSeq>90</><ChannelSeq>398243</><ChkCode>cweb</><MTimeStamp>2013-07-24 09:55:28:967000</><NoteMsg></><RspCode>000000</><RspMsg>成功</><STimeStamp>2015-07-24 09:55:28:966000</><TellCode>cweb</><TranChannel>00</><TranCode>116005</><TranDate>2016-04-06</><TranIP>10.22.21.26</><TranSeq>90</><fileName>YEMXCX.txt</><accnum></><accname></><sumcounts></><counts></><pageflag>1</>";
			...
			if(!app002Result.getFileName().equals("")){
				...
				int transdate=0,freeuse1=0,amt=0,remark=0;
				for(int i=0;i<paraNames.length;i++){
					if("transdate".equals(paraNames[i])){
						transdate = i;//交易日期
					}else if("freeuse1".equals(paraNames[i])){
						freeuse1 = i;//业务类型
...
//					return modelMap;
				...
				while (line != null) {
					QvZhiAppApi002Result aa00201Result = new QvZhiAppApi002Result();
					try {
						String[] param = line.split("~");									
						log.debug("读取文件  ："+line);
						////String.format("%,.2f",Double.valueOf(appApi00401Result.getAmt1()))
						aa00201Result.setTransdate(param[transdate].trim());
						aa00201Result.setFreeuse1(param[freeuse1].trim());	
						aa00201Result.setAmt(String.format("%,.2f",Double.valueOf(param[amt])));	
						if(param.length>remark)
						{
							aa00201Result.setRemark(param[remark].trim());	
						}else
						{
							aa00201Result.setRemark("--");	
						}
